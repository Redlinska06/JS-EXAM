...
//shuffling cards and picking 5 on your hand
let yourHand = [];
for (let i = 0; i <= 4; i++) {
  // draw card from deck
  let newCard = Math.floor(Math.random() * deck.length);
  // pick card to player hand
...
// Flush - Kolor to karty w tym samym kolorze, ale występujące nie po kolei, na przykład: Król, Jopek, 6, 2, Dama.
  ...
  console.log(`Congratulations you have Flush ${cardsSuits[0].suit}`);
}
// Straight - Strit to pięć kart po kolei, na przykład 5, 6, 7, 8, 9.
  ...
  console.log(
    `Congratulations you have Straight from ${cardsSorted[4].rank} to ${cardsSorted[0].rank}`
  );
} else if (
  cardsSorted[0].rank == cardsSorted[1].rank - 1 &&
  cardsSorted[1].rank == cardsSorted[2].rank - 1 &&
  ...
  console.log(
    `Congratulations you have Straight from ${cardsSorted[0].rank} to ${cardsSorted[4].rank}`
  );
}
